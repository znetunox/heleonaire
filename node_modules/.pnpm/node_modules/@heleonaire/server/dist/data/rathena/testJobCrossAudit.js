...
function main() {
    ...
    statsGroups.forEach((group, index) => {
        for (const jobName of group.jobs) {
            jobToGroup.set(normalizeJobName(jobName), index + 1);
        }
    });
    const matched = [];
    const missingFromStats = [];
    for (const job of jobResult.jobs) {
        const normalizedName = normalizeJobName(job.aegisName);
    ...
    console.log(`job_stats groups:    ${statsGroups.length}`);
    console.log(`job_stats jobs:      ${jobToGroup.size}`);
    console.log(`Matched jobs:        ${matched.length}`);
    console.log(`Missing from stats:  ${missingFromStats.length}`);
    console.log("");
    console.log("----------------------------------------");
    ...
    for (const job of matched) {
        console.log(`${String(job.id).padEnd(8)} ` +
    ...
    for (const job of missingFromStats) {
        console.log(`${String(job.id).padEnd(8)} ${job.aegisName}`);
    }
    console.log("");
    console.log("----------------------------------------");
    ...
    for (const name of playable) {
        const job = jobResult.jobs.find((entry) => normalizeJobName(entry.aegisName) === name);
        const group = job
            ? jobToGroup.get(normalizeJobName(job.aegisName))
            : undefined;
