...
for (const group of groups) {
    ...
    if (group.entries.length !== group.maxJobLevel) {
        console.error(
            `  ERROR: entries=${group.entries.length}, max=${group.maxJobLevel}`
        );
    ...
    for (let i = 0; i < group.entries.length; i++) {
        ...
        if (entry.level !== i + 1) {
            console.error(
                `  ERROR: expected level ${i + 1}, got ${entry.level}`
            );
        ...
        if (entry.exp < 0n) {
            console.error(
                `  ERROR: negative EXP at level ${entry.level}`
            );
    ...
    console.log(
        `  First: Level ${first.level} -> ${first.exp} EXP`
    );

    console.log(
        `  Last: Level ${last.level} -> ${last.exp} EXP`
    );
