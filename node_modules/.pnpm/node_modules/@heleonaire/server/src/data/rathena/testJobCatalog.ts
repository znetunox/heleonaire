import {
  getHeleonaireProgression,
  parseJobCatalog,
} from "./jobCatalogParser";
...
console.log(
  `Catalog jobs:          ${result.jobs.length}`
);

console.log(
  `Playable jobs:         ${result.playableJobs.length}`
);

console.log(
  `Missing mmo.hpp:       ${result.missingFromMmo.length}`
);

console.log(
  `Missing stats:         ${result.missingStats.length}`
);

console.log(
  `Missing skill tree:    ${result.missingSkillTree.length}`
);

console.log(
  `Invalid parents:       ${result.invalidParents.length}`
);

console.log(
  `Duplicate jobs:        ${result.duplicateJobs.length}`
);
...
assert(
  result.missingFromMmo.length === 0,
  `Some catalog jobs are missing from mmo.hpp: ${result.missingFromMmo.join(", ")}`
);

assert(
  result.missingStats.length === 0,
  `Some catalog jobs are missing from job_stats.yml: ${result.missingStats.join(", ")}`
);

assert(
  result.missingSkillTree.length === 0,
  `Some catalog jobs are missing from skill_tree.yml: ${result.missingSkillTree.join(", ")}`
);

assert(
  result.invalidParents.length === 0,
  `Invalid parent relationships: ${result.invalidParents.join(", ")}`
);

assert(
  result.duplicateJobs.length === 0,
  `Duplicate jobs found: ${result.duplicateJobs.join(", ")}`
);

assert(
  result.jobs.length === 25,
  `Expected exactly 25 playable jobs, got ${result.jobs.length}`
);

assert(
  result.branches.length === 5,
  `Expected 5 branches, got ${result.branches.length}`
);

for (const branch of result.branches) {
  assert(
    branch.jobs.length === 5,
    `${branch.heleonaireClass} should contain exactly 5 jobs, got ${branch.jobs.length}`
  );
}
...
function assertParent(
  ...
  assert(
    !!job,
    `${jobName} was not found`
  );

  assert(
    job.parentJob === expectedParent,
    `${jobName} expected parent ${expectedParent ?? "none"}, got ${job.parentJob ?? "none"}`
  );
}
...
for (const branch of getHeleonaireProgression()) {
  console.log(
    `${branch.heleonaireClass}: ${branch.jobs.join(" -> ")}`
  );
}
