...
export async function importJobExpData(): Promise<void> {
  console.log("");
  console.log("========================================");
  ...
  if (playableJobs.length !== 25) {
    throw new Error(
      `[JobExpImporter] Expected 25 playable jobs, got ${playableJobs.length}`
    );
  }
  ...
  for (const group of groups) {
    for (const rawJob of group.jobs) {
      ...
      if (groupByJob.has(jobName)) {
        throw new Error(
          `[JobExpImporter] Job belongs to multiple Job EXP groups: ${jobName}`
        );
      }
  ...
  for (const job of playableJobs) {
    ...
    if (!groupByJob.has(jobName)) {
      throw new Error(
        `[JobExpImporter] Job EXP group not found for ${jobName}`
      );
    }
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const group of groups) {
      ...
      console.log(
        `[JobExpImporter] Group #${group.index} | ` +
    ...
    for (const job of playableJobs) {
      ...
      if (!sourceGroup) {
        throw new Error(
          `[JobExpImporter] Source group missing for ${jobName}`
        );
      }
      ...
      if (!group) {
        throw new Error(
          `[JobExpImporter] Persisted group not found for ${jobName}`
        );
      }
      ...
      if (!gameClass) {
        throw new Error(
          `[JobExpImporter] GameClass not found: ${job.job}`
        );
      }
