...
import { parseItems, ParsedItem } from "../parsers/itemParser";
...
function calculateSourceHash(item: ParsedItem): string {
...
async function importItem(item: ParsedItem): Promise<void> {
...
export async function importItems(): Promise<ItemImportResult> {
  ...
  for (const item of result.items.values()) {
    try {
      ...
      if (imported % 500 === 0) {
        console.log(
          `[ItemImporter] Progress: ${ imported }/${result.items.size}`
        );
      }
    } catch (error) {
    failed++;
    ...
    console.error(
        `[ItemImporter] Failed: item=${item.id} (${item.aegisName})`
    );
