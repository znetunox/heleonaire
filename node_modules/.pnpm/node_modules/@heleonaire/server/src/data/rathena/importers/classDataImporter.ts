...
function buildStatsGroupMap() {
  ...
  const map = new Map<
    ...
    {
      groupIndex: number;
      group: (typeof parsed.groups)[number];
    }
  >();
...
export async function importClassData() {
  ...
  if (catalog.jobs.length !== 25) {
    throw new Error(
      `[ClassImporter] Expected 25 Heleonaire jobs, got ${catalog.jobs.length}`
    );
  }

  if (catalog.missingFromMmo.length > 0) {
    throw new Error(
      `[ClassImporter] Missing mmo.hpp jobs: ${catalog.missingFromMmo.join(", ")}`
    );
  }

  if (catalog.missingStats.length > 0) {
    throw new Error(
      `[ClassImporter] Missing job_stats jobs: ${catalog.missingStats.join(", ")}`
    );
  }

  if (catalog.missingSkillTree.length > 0) {
    throw new Error(
      `[ClassImporter] Missing skill_tree jobs: ${catalog.missingSkillTree.join(", ")}`
    );
  }
  ...
  if (missingAspd.length > 0) {
    throw new Error(
      `[ClassImporter] Missing job_aspd jobs: ${missingAspd.join(", ")}`
    );
  }
  ...
  for (const job of catalog.jobs) {
    ...
    if (!statsEntry) {
      throw new Error(
        `[ClassImporter] Stats group not found for ${jobName}`
      );
    }
  ...
  for (const job of catalog.jobs) {
    ...
    if (!weaponAspd) {
      throw new Error(
        `[ClassImporter] ASPD data not found for ${jobName}`
      );
    }
    ...
    const rows = Array.from(weaponAspd.entries()).map(
      ([weaponType, aspd]) => ({
        classId: job.id,
        weaponType,
        aspd,
