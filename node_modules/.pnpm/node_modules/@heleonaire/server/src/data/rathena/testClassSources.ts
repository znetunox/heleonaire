...
console.log(
  `Entries: ${statPoints.entries.length}`
);

for (const entry of statPoints.entries.slice(0, 10)) {
  console.log(
    `Lv ${entry.level} | points=${entry.points} | traitPoints=${entry.traitPoints ?? 0}`
  );
}
...
console.log(
  `Groups: ${aspd.groups.length}`
);

console.log(
  `Jobs: ${jobsWithAspd.size}`
);

console.log(
  `ASPD entries: ${totalAspdEntries}`
);
...
if (firstGroup) {
  console.log("");
  console.log(
    `First group jobs: ${firstGroup.jobs.join(", ")}`
  );

  for (const [
    weapon,
    value,
  ] of firstGroup.baseASPD) {
    console.log(
      `  ${weapon}: ${value}`
    );
  }
