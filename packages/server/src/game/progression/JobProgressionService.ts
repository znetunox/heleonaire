...
const MAX_LEVEL_JOB_EXP = 999_999_999n;
export interface JobExpGainResult {
    characterId: string;
...
class JobProgressionService {
    ...
    async gainJobExp(
        ...
        return prisma.$transaction(async tx => {
             ...
             */
            const currentEntry =
                await tx.jobExpEntry.findUnique({
                    where: {
                        groupId_level: {
            ...
            let updatedRequiredJobExp:
                bigint | null = null;

            if (
                updatedCharacter.jobLevel <
                maxJobLevel
            ) {
                const nextEntry =
                    await tx.jobExpEntry.findUnique({
            ...
            return {
                ...
                jobLevel:
                    updatedCharacter.jobLevel,
                jobExp:
                    updatedCharacter.jobExp,
                availableSkillPoints:
                    updatedCharacter.availableSkillPoints,
                jobLevelUps,
                requiredJobExp:
                    updatedRequiredJobExp,
                maxJobLevel,
    ...
    async getCurrentProgression(
        ...
        if (gameClass.jobExpGroupId === null) {
            throw new Error(
                "JOB_EXP_GROUP_NOT_FOUND",
            );
        }

        const jobExpGroup =
            await prisma.jobExpGroup.findUnique({
                where: {
                    id: gameClass.jobExpGroupId,
                },
            });
        ...
        const maxJobLevel =
            jobExpGroup.maxJobLevel;

        let requiredJobExp:
            bigint | null = null;

        if (
            character.jobLevel < maxJobLevel
        ) {
            const entry =
                await prisma.jobExpEntry.findUnique({
                    where: {
                        groupId_level: {
                            groupId:
                                jobExpGroup.id,
