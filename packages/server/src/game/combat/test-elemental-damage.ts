...
(() => {
    const result = calculateElementalDamage(
        100,
        "Neutral",
        "Neutral",
        1,
    ...
    if (result.preElementDamage !== 100) {
        throw new Error(
            `Neutral→Neutral preElementDamage: expected 100, got ${result.preElementDamage}`,
        );
    }

    if (result.postElementDamage !== 100) {
        throw new Error(
            `Neutral→Neutral postElementDamage: expected 100, got ${result.postElementDamage}`,
        );
    }

    if (result.elementRatio !== 100) {
        throw new Error(
            `Neutral→Neutral ratio: expected 100, got ${result.elementRatio}`,
        );
    }
...
(() => {
    ...
    if (result.preElementDamage !== 100) {
        throw new Error(
            `Fire→Fire preElementDamage: expected 100, got ${result.preElementDamage}`,
        );
    }
    ...
    if (result.postElementDamage >= result.preElementDamage) {
        throw new Error(
            `Fire→Fire postElementDamage: expected < 100, got ${result.postElementDamage}`,
        );
    }

    if (result.elementRatio >= 100) {
        throw new Error(
            `Fire→Fire ratio: expected < 100, got ${result.elementRatio}`,
        );
    }
...
(() => {
    ...
    if (result.preElementDamage !== 100) {
        throw new Error(
            `Fire→Earth preElementDamage: expected 100, got ${result.preElementDamage}`,
        );
    }
    ...
    if (result.postElementDamage <= result.preElementDamage) {
        throw new Error(
            `Fire→Earth postElementDamage: expected > 100, got ${result.postElementDamage}`,
        );
    }

    if (result.elementRatio <= 100) {
        throw new Error(
            `Fire→Earth ratio: expected > 100, got ${result.elementRatio}`,
        );
    }
...
(() => {
    ...
    if (result.preElementDamage !== 100) {
        throw new Error(
            `Water→Fire preElementDamage: expected 100, got ${result.preElementDamage}`,
        );
    }
    ...
    if (result.postElementDamage <= result.preElementDamage) {
        throw new Error(
            `Water→Fire postElementDamage: expected > 100, got ${result.postElementDamage}`,
        );
    }

    if (result.elementRatio <= 100) {
        throw new Error(
            `Water→Fire ratio: expected > 100, got ${result.elementRatio}`,
        );
    }
...
(() => {
    ...
    for (const [attackElement, targetElement, level] of elements) {
        ...
        if (result.preElementDamage !== 100) {
            throw new Error(
                `${attackElement}→${targetElement} L${level} preElementDamage: expected 100, got ${result.preElementDamage}`,
            );
        }

        if (result.targetElementLevel !== level) {
            throw new Error(
                `${attackElement}→${targetElement} L${level} level: expected ${level}, got ${result.targetElementLevel}`,
            );
        }
    }
...
(() => {
    const originalComponents: AttackComponents = {
        statusAtk: 100,
        weaponAtk: 50,
        equipAtk: 25,
        masteryAtk: 75,
    ...
    if (context.attackElement !== "Fire") {
        throw new Error(
            `Context attackElement: expected Fire, got ${context.attackElement}`,
        );
    }

    if (context.targetElement !== "Earth") {
        throw new Error(
            `Context targetElement: expected Earth, got ${context.targetElement}`,
        );
    }

    if (context.targetElementLevel !== 1) {
        throw new Error(
            `Context level: expected 1, got ${context.targetElementLevel}`,
        );
    }
...
(() => {
    ...
    const result = calculateElementalAttackComponents(
        originalComponents,
        20, // batk
        "Fire",
        "Earth",
    ...
    if (result.input.statusAtk !== originalComponents.statusAtk) {
        throw new Error(
            `Input statusAtk: expected ${originalComponents.statusAtk}, got ${result.input.statusAtk}`,
        );
    }

    if (result.output.statusAtk !== originalComponents.statusAtk) {
        throw new Error(
            `Output statusAtk: expected ${originalComponents.statusAtk}, got ${result.output.statusAtk}`,
        );
    }

    if (result.output.weaponAtk !== originalComponents.weaponAtk) {
        throw new Error(
            `Output weaponAtk: expected ${originalComponents.weaponAtk}, got ${result.output.weaponAtk}`,
        );
    }
    ...
    if (result.context.attackElement !== "Fire") {
        throw new Error(
            `Context attackElement: expected Fire, got ${result.context.attackElement}`,
        );
    }
...
(() => {
    ...
    if (fireEarthRatio <= 100) {
        throw new Error(
            `Fire→Earth ratio: expected > 100, got ${fireEarthRatio}`,
        );
    }
    ...
    if (fireFireRatio >= 100) {
        throw new Error(
            `Fire→Fire ratio: expected < 100, got ${fireFireRatio}`,
        );
    }
    ...
    if (neutralNeutralRatio !== 100) {
        throw new Error(
            `Neutral→Neutral ratio: expected 100, got ${neutralNeutralRatio}`,
        );
    }
...
(() => {
    ...
    const result1 = calculateElementalDamage(
        100,
        "Fire",
        "Earth",
        1,
    ...
    if (result1.postElementDamage <= result1.preElementDamage) {
        throw new Error(
            `Fórmula com ratio > 100: expected post > pre, got ${result1.postElementDamage} <= ${result1.preElementDamage}`,
        );
    }

    // Teste com ratio < 100 (reduz)
    const result2 = calculateElementalDamage(
        100,
        "Fire",
        "Fire",
        1,
    ...
    if (result2.postElementDamage >= result2.preElementDamage) {
        throw new Error(
            `Fórmula com ratio < 100: expected post < pre, got ${result2.postElementDamage} >= ${result2.preElementDamage}`,
        );
    }
