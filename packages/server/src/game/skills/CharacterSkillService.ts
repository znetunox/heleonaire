...
class CharacterSkillService {
     ...
     */
    async getCharacterSkills(characterId: string) {
    ...
    async canLearnSkill(
        ...
        if (classSkill.prerequisites.length > 0) {
            const requiredSkillIds = classSkill.prerequisites.map(
                prerequisite => prerequisite.requiredSkillId
            );
            ...
            for (const prerequisite of classSkill.prerequisites) {
                const learnedSkill = learnedSkills.find(
                    entry =>
                        entry.skillId === prerequisite.requiredSkillId
                );
    ...
    async learnSkill(
        ...
        return prisma.$transaction(async tx => {
             ...
             */
            for (const prerequisite of classSkill.prerequisites) {
                const requiredCharacterSkill =
                    await tx.characterSkill.findUnique({
                        where: {
                            characterId_skillId: {
                                characterId,
                ...
                const requiredLevel =
                    requiredCharacterSkill?.level ?? 0;
