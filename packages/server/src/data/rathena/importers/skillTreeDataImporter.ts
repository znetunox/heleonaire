...
import {
    ...
    type EffectiveSkillTreeSkill,
    type ParsedSkillTreeJob,
...
function validateEffectiveSkill(
    skill: EffectiveSkillTreeSkill,
    jobName: string,
): void {
    if (!skill.name) {
        throw new Error(
            `Skill without name found in effective tree of ${jobName}.`,
        );
    }

    if (skill.maxLevel <= 0) {
        throw new Error(
            `Invalid effective MaxLevel for ${jobName} -> ${skill.name}: ${skill.maxLevel}`,
        );
    }

    if (
        skill.baseLevel !== undefined &&
        skill.baseLevel < 0
    ) {
        throw new Error(
            `Invalid BaseLevel for ${jobName} -> ${skill.name}: ${skill.baseLevel}`,
        );
    }

    if (
        skill.jobLevel !== undefined &&
        skill.jobLevel < 0
    ) {
        throw new Error(
            `Invalid JobLevel for ${jobName} -> ${skill.name}: ${skill.jobLevel}`,
        );
    }
    ...
    for (const requirement of skill.requires) {
        if (!requirement.name) {
            throw new Error(
                `Empty prerequisite skill name in ${jobName} -> ${skill.name}.`,
            );
        }
...
async function importSkillTreeData(): Promise<void> {
    console.log(
        "[rAthena] Starting skill tree import...",
    );
    ...
    console.log(
        `[rAthena] skill_tree jobs parsed: ${parsed.jobs.length}`,
    );

    console.log(
        `[rAthena] skill_tree duplicates: ${parsed.duplicates}`,
    );

    if (parsed.duplicates > 0) {
        throw new Error(
            `skill_tree.yml contains ${parsed.duplicates} duplicate job(s).`,
        );
    }
    ...
    for (const jobName of HELEONAIRE_JOBS) {
        ...
        totalPrerequisites += effective.reduce(
            (total, skill) =>
                total + skill.requires.length,
            0,
        );

        console.log(
            `[rAthena] ${jobName.padEnd(18)} ${effective.length} effective skills`,
        );
    }
    ...
    console.log(
        `[rAthena] total effective ClassSkill records: ${totalClassSkills}`,
    );

    console.log(
        `[rAthena] total ClassSkillPrerequisite records: ${totalPrerequisites}`,
    );
    ...
    const missingDbClasses =
        HELEONAIRE_JOBS.filter(
            (jobName) =>
                !dbClassMap.has(jobName),
        );
    ...
    if (missingPrerequisites.length > 0) {
        const details =
            missingPrerequisites
                ...
                .map(
                    (entry) =>
                        `${entry.job} -> ${entry.skill} requires ${entry.requiredSkill}`,
                )
                .join(", ");
    ...
    try {
        await prisma.$transaction(
            async (tx) => {
                ...
                const deletedClassSkills =
                    await tx.classSkill.deleteMany({
                        where: {
                            classId: {
                                in: classIds,
                            },
                ...
                for (const jobName of HELEONAIRE_JOBS) {
                    const classId =
                        dbClassMap.get(jobName);

                    if (classId === undefined) {
                        throw new Error(
                            `GameClass not found during transaction: ${jobName}`,
                        );
                    }
                    ...
                    for (const skill of effective) {
                        ...
                        if (
                            ...
                            throw new Error(
                                `Skill not found during transaction: ${skill.name}`,
                            );
                        }
        ...
        console.log(
            "Skill Tree import completed successfully",
        );
        console.log(
            "==============================================",
        );
        ...
        console.log(
            `Playable jobs:             ${HELEONAIRE_JOBS.length}`,
        );
        ...
        console.log(
            `Prerequisite records:      ${totalPrerequisites}`,
        );
