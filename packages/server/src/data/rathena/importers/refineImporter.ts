...
async function resolveMaterials(
  ...
  for (const name of requiredNames) {
    const itemId =
      itemMap.get(normalizeName(name));

    if (itemId === undefined) {
      throw new Error(
        `Failed to resolve refine material item: ${name}`
      );
    }
...
function validateRules(
  ...
  if (rules.length === 0) {
    throw new Error(
      "refine.yml produced zero refine rules. Import aborted."
    );
  }
  ...
  for (const rule of rules) {
    if (!rule.group.trim()) {
      throw new Error(
        "Refine rule has an empty Group."
      );
    }
    ...
    if (keys.has(key)) {
      throw new Error(
        `Duplicate refine rule detected: ${key}`
      );
    }

    keys.add(key);

    const chanceTypes = new Set<string>();

    for (const chance of rule.chances) {
      if (!chance.type.trim()) {
        throw new Error(
          `Refine rule ${key} contains a chance without Type.`
        );
      }

      if (!chance.material.trim()) {
        throw new Error(
          `Refine rule ${key} contains a chance without Material.`
        );
      }

      if (chanceTypes.has(chance.type)) {
        throw new Error(
          `Duplicate refine chance type "${chance.type}" in rule ${key}.`
        );
      }
...
async function importRefineData() {
  console.log(
    "[rAthena] Starting refine DB import..."
  );

  const parsed = parseRefine();

  console.log(
    `[rAthena] refine.yml source: rathena`
  );

  console.log(
    `[rAthena] refine rules parsed: ${parsed.rules.length}`
  );

  console.log(
    `[rAthena] refine duplicates: ${parsed.duplicates}`
  );
  ...
  const materialMap =
    await resolveMaterials(parsed.rules);

  console.log(
    `[rAthena] refine material items resolved: ${materialMap.size}`
  );
  ...
  const expectedChances =
    parsed.rules.reduce(
      (total, rule) =>
        total + rule.chances.length,
      0
    );

  console.log(
    `[rAthena] expected RefineRule records: ${expectedRules}`
  );

  console.log(
    `[rAthena] expected RefineChance records: ${expectedChances}`
  );
  ...
  try {
    await prisma.$transaction(
      async (tx) => {
        ...
        if (staleRuleIds.length > 0) {
          const deleted =
            await tx.refineRule.deleteMany({
              where: {
                id: {
                  in: staleRuleIds,
                },
        ...
        for (const rule of parsed.rules) {
          ...
          const existing =
            await tx.refineRule.findUnique({
              where: {
          ...
          await tx.refineRule.upsert({
            ...
            create: {
              ...
              blacksmithBlessingAmount:
                rule.blacksmithBlessingAmount,

              broadcastSuccess:
                rule.broadcastSuccess,

              broadcastFailure:
                rule.broadcastFailure,
        ...
        for (const rule of parsed.rules) {
          ...
          const existingChances =
            await tx.refineChance.findMany({
              where: {
                refineRuleId:
                  refineRule.id,
              },
          ...
          const staleChanceIds =
            existingChances
              .filter(
                (chance) =>
                  !importedChanceTypes.has(
                    chance.type
                  )
              )
              .map(
          ...
          if (staleChanceIds.length > 0) {
            const deleted =
              await tx.refineChance.deleteMany({
                where: {
                  id: {
                    in: staleChanceIds,
                  },
          ...
          for (const chance of rule.chances) {
            ...
            const existingChance =
              await tx.refineChance.findUnique({
                where: {
                  refineRuleId_type: {
                    refineRuleId:
                      refineRule.id,
    ...
    console.log(
      "Refine DB import completed successfully"
    );
    console.log(
      "=============================================="
    );
    ...
    console.log(
      `Refine rules:          ${expectedRules}`
    );

    console.log(
      `Refine chances:        ${expectedChances}`
    );
