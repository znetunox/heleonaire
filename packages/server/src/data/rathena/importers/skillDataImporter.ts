...
async function resolveRequiredItems(
  skillDefinitions: Awaited<ReturnType<typeof parseSkillDb>>["skills"]
) {
  ...
  const normalizedNames = new Set(
    [...requiredNames].map(normalizeName)
  );
  ...
  for (const name of requiredNames) {
    ...
    if (itemId === undefined) {
      throw new Error(
        `Failed to resolve skill requirement item: ${name}`
      );
    }
...
async function importSkillData() {
  ...
  console.log(
    `[rAthena] skill_db.yml source: rathena`
  );

  console.log(
    `[rAthena] skills parsed: ${parsed.skills.length}`
  );

  console.log(
    `[rAthena] skill duplicates: ${parsed.duplicates}`
  );

  if (parsed.duplicates > 0) {
    throw new Error(
      `skill_db.yml contains ${parsed.duplicates} duplicate skill ID(s).`
    );
  }

  if (parsed.skills.length === 0) {
    throw new Error(
      "skill_db.yml produced zero skills. Import aborted."
    );
  }
  ...
  for (const skill of parsed.skills) {
    if (skillIds.has(skill.id)) {
      throw new Error(
        `Duplicate skill ID detected: ${skill.id}`
      );
    }

    if (skillNames.has(skill.aegisName)) {
      throw new Error(
        `Duplicate skill AegisName detected: ${skill.aegisName}`
      );
    }

    if (skill.id <= 0) {
      throw new Error(
        `Invalid skill ID: ${skill.id} (${skill.aegisName})`
      );
    }

    if (skill.maxLevel <= 0) {
      throw new Error(
        `Invalid MaxLevel for ${skill.aegisName}: ${skill.maxLevel}`
      );
    }
  ...
  console.log(
    `[rAthena] skill requirement items resolved: ${itemMap.size}`
  );
  ...
  const expectedLevels = parsed.skills.reduce(
    (total, skill) => total + skill.levels.length,
    0
  );

  const expectedRequirements = parsed.skills.reduce(
    (total, skill) => total + skill.requirements.length,
    0
  );

  const expectedRequirementItems = parsed.skills.reduce(
    (total, skill) =>
      total +
      skill.requirements.reduce(
        (reqTotal, requirement) =>
          reqTotal + requirement.itemCosts.length,
        0
      ),
    0
  );

  const expectedUnits = parsed.skills.reduce(
    (total, skill) => total + skill.units.length,
    0
  );

  const expectedUnitLevels = parsed.skills.reduce(
    (total, skill) =>
      total +
      skill.units.reduce(
        (unitTotal, unit) =>
  ...
  console.log(
    `[rAthena] expected Skill records: ${parsed.skills.length}`
  );

  console.log(
    `[rAthena] expected SkillLevel records: ${expectedLevels}`
  );

  console.log(
    `[rAthena] expected SkillRequirement records: ${expectedRequirements}`
  );

  console.log(
    `[rAthena] expected SkillRequirementItem records: ${expectedRequirementItems}`
  );

  console.log(
    `[rAthena] expected SkillUnit records: ${expectedUnits}`
  );

  console.log(
    `[rAthena] expected SkillUnitLevel records: ${expectedUnitLevels}`
  );
  ...
  try {
    await prisma.$transaction(
      async (tx) => {
        ...
        const oldRequirementItems =
          await tx.skillRequirementItem.deleteMany({
            where: {
              requirement: {
                skillId: {
                  in: ids,
        ...
        const oldUnitLevels =
          await tx.skillUnitLevel.deleteMany({
            where: {
              skillUnit: {
                skillId: {
                  in: ids,
        ...
        for (const skill of parsed.skills) {
          ...
          await tx.skillLevel.createMany({
            data: skill.levels.map((level) => ({
              ...
              afterCastActDelay:
                level.afterCastActDelay,
              afterCastWalkDelay:
                level.afterCastWalkDelay,
        ...
        for (const skill of parsed.skills) {
          for (const requirement of skill.requirements) {
            const createdRequirement =
              await tx.skillRequirement.create({
                data: {
                  skillId: skill.id,
                  ...
                  hpRateCost:
                    requirement.hpRateCost,
                  spRateCost:
                    requirement.spRateCost,
                  apRateCost:
                    requirement.apRateCost,

                  maxHpTrigger:
                    requirement.maxHpTrigger,

                  zenyCost:
                    requirement.zenyCost,

                  weapon: requirement.weapon,
                  ammo: requirement.ammo,
                  ammoAmount:
                    requirement.ammoAmount,

                  state: requirement.state,
                  status: requirement.status,

                  spiritSphereCost:
                    requirement.spiritSphereCost,
            ...
            if (requirement.itemCosts.length > 0) {
              const itemCosts =
                requirement.itemCosts.map(
                  (itemCost) => {
                    const itemId =
                      itemMap.get(itemCost.item);
    ...
    console.log(
      `Requirement items:     ${expectedRequirementItems}`
    );

    console.log(
      `Skill units:           ${expectedUnits}`
    );

    console.log(
      `Unit levels:           ${expectedUnitLevels}`
    );
