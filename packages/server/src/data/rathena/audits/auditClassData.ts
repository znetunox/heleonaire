...
async function auditClassData() {
  ...
  if (classes.length !== 25) {
    console.error(
      `[ERROR] Expected 25 GameClass records, got ${classes.length}`
    );
    errors++;
  }

  const playableClasses = classes.filter(
    (gameClass) => gameClass.heleonaireClass !== null
  );

  console.log(
    `[Audit] Playable class records: ${playableClasses.length}`
  );

  if (playableClasses.length !== 25) {
    console.error(
      `[ERROR] Expected 25 playable classes, got ${playableClasses.length}`
    );
    errors++;
  }
  ...
  console.log(
    `[Audit] ClassStatGrowth count: ${statGrowthCount}`
  );

  if (statGrowthCount !== 956) {
    console.error(
      `[ERROR] Expected 956 ClassStatGrowth records, got ${statGrowthCount}`
    );
    errors++;
  }
  ...
  if (aspdCount !== 182) {
    console.error(
      `[ERROR] Expected 182 ClassAspd records, got ${aspdCount}`
    );
    errors++;
  }
  ...
  console.log(
    `[Audit] StatPointTable count: ${statPointCount}`
  );

  if (statPointCount !== 275) {
    console.error(
      `[ERROR] Expected 275 StatPointTable records, got ${statPointCount}`
    );
    errors++;
  }
  ...
  if (!knight) {
    ...
    for (const row of knight.statGrowth) {
      console.log(
        `  Lv ${String(row.level).padStart(2)} | ` +
    ...
    for (const row of knight.aspd) {
      console.log(
        `  ${row.weaponType.padEnd(20)} | ${row.aspd}`
      );
    }
  }
  ...
  for (const row of statPoints) {
    console.log(
      `  Lv ${String(row.level).padStart(3)} | ` +
