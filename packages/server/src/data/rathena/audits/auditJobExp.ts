...
const resolved = resolveYaml<RawJobExpFile>("job_exp.yml");
...
for (const job of playableJobs) {
    ...
    const matches = groups
        .map((group, index) => ({
            ...
            group,
            matches:
                Array.isArray(group.JobExp) &&
                group.JobExp.length > 0 &&
                !!group.Jobs &&
                Object.keys(group.Jobs).some(
                    name => normalizeJobName(name) === normalizedTarget
                ),
        }))
    ...
    if (matches.length !== 1) {
        errors.push(
            `${job.job}: esperado exatamente 1 grupo, encontrado ${matches.length}`
        );
        continue;
    }
    ...
    if (maxJobLevel !== undefined && entries.length !== maxJobLevel) {
        errors.push(
            `${job.job}: entries=${entries.length}, MaxJobLevel=${maxJobLevel}`
        );
    }
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        if (level !== i + 1) {
            errors.push(
                `${job.job}: posição ${i + 1} contém Level ${level}`
            );
        }

        if (exp < 0) {
            errors.push(
                `${job.job}: Level ${level}: EXP negativa (${exp})`
            );
        }
    }

    const duplicateLevels = levels.filter(
        (level, i) => levels.indexOf(level) !== i
    );

    if (duplicateLevels.length > 0) {
        errors.push(
            `${job.job}: níveis duplicados: ${[
    ...
    if (levels.length > 0) {
        console.log(
            `  First: Level ${levels[0]} -> ${expValues[0]} EXP`
        );
        console.log(
            `  Last:  Level ${levels[levels.length - 1]} -> ${
                expValues[expValues.length - 1]
            } EXP`
        );
        ...
        for (let i = 0; i < levels.length; i++) {
            console.log(
                `    Job ${levels[i]} -> ${expValues[i].toLocaleString()} EXP`
            );
        }
    }
