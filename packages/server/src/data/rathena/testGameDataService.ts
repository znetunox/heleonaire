...
async function testGameDataService(): Promise<void> {
    ...
    for (const drop of drops) {
        ...
        if (!item) {
            throw new Error(
                `Item ${drop.itemId} do drop ${drop.sourceKey} não encontrado.`
            );
        }
    ...
    if (drops.length !== 8) {
        throw new Error(
            `Poring deveria possuir 8 drops, mas possui ${drops.length}.`
        );
    }
    ...
    if (appleDrops.length !== 2) {
        throw new Error(
            `Poring deveria possuir 2 entradas de Apple, mas possui ${appleDrops.length}.`
        );
    }
    ...
    if (
        ...
        throw new Error(
            `Rates de Apple incorretos: ${appleRates.join(", ")}`
        );
    }
    ...
    if (!cardDrop.stealProtected) {
        throw new Error(
            "Poring Card deveria possuir stealProtected=true."
        );
    }
