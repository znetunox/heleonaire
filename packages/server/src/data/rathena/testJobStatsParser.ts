...
function main() {
  ...
  const duplicatedJobs: string[] = [];

  for (const [job, groups] of jobGroups) {
    if (groups.length > 1) {
      duplicatedJobs.push(
        `${job} -> groups ${groups.join(", ")}`
      );
    }
  }

  console.log(
    `Jobs in multiple groups: ${duplicatedJobs.length}`
  );
  ...
  result.groups.forEach((group, index) => {
    ...
    console.log(
      `     BonusStats: ${group.bonusStats.length}`
    );

    console.log(
      `     ASPD: ${group.baseASPD.size}`
    );
