...
import { calcDef, } from "@heleonaire/shared";
import { CombatStateBuilder, } from "../game/combat/CombatStateBuilder";
...
export class WorldRoom extends Room {
    constructor() {
        ...
        this.processingDropPickups = new Set();
        this.statSystem = new StatSystem(prisma);
        this.combatStateBuilder = new CombatStateBuilder(prisma, statusService);
        this.characterSkillService = new CharacterSkillService();
        this.jobProgressionService = new JobProgressionService();
    }
    spawnGroundDrop(itemId, itemName, quantity, x, y, ownerId) {
        ...
        console.log(`[WorldRoom] GroundDrop spawned: ` +
            `${itemName} (${itemId}) ` +
    ...
    cleanupExpiredDrops() {
        ...
        this.state.drops.forEach((drop, dropId) => {
            if (drop.expiresAt <= now) {
                console.log(`[WorldRoom] GroundDrop expired: ` +
    ...
    // GROUND DROP PICKUP
    // ─────────────────────────────────────────────────────────────
    async handlePickupDrop(client, message) {
        // ─────────────────────────────────────────────────────────
        // PLAYER
        // ─────────────────────────────────────────────────────────
        const player = this.state.players.get(client.sessionId);
        if (!player) {
            console.warn(`[WorldRoom] pickupDrop rejected: player not found ` +
        ...
        if (!dropId ||
            ...
            console.warn(`[WorldRoom] pickupDrop rejected: invalid dropId ` +
                `session=${client.sessionId}`);
            return;
        }
        // ─────────────────────────────────────────────────────────
        ...
        if (!drop) {
            console.warn(`[WorldRoom] pickupDrop rejected: drop not found ` +
                `drop=${dropId} ` +
                `player=${player.name}`);
            return;
        }
        ...
        if (drop.expiresAt <= now) {
            this.state.drops.delete(dropId);
            console.log(`[WorldRoom] pickupDrop rejected: drop expired ` +
                `drop=${dropId} ` +
        ...
        if (distanceSquared >
            ...
            console.warn(`[WorldRoom] pickupDrop rejected: too far ` +
                `player=${player.name} ` +
                `drop=${dropId} ` +
        ...
        // OWNERSHIP
        // ─────────────────────────────────────────────────────────────
        if (drop.ownershipExpiresAt > now &&
            drop.ownerId !== player.id) {
            console.warn(`[WorldRoom] pickupDrop rejected: not owner ` +
                `player=${player.name} ` +
                `drop=${dropId}`);
            return;
        }
        ...
        if (this.processingDropPickups.has(dropId)) {
            console.warn(`[WorldRoom] pickupDrop rejected: ` +
                `already processing ` +
                `drop=${dropId} ` +
                `player=${player.name}`);
            return;
        ...
        try {
            const added = await inventoryService.addItem(player.characterId, drop.itemId, drop.quantity);
            if (!added) {
                console.warn(`[WorldRoom] pickupDrop failed: ` +
            ...
            // SYNC INVENTORY
            // ─────────────────────────────────────────────────────────────
            const inventory = await inventoryService.getInventory(player.characterId);
            client.send("inventorySnapshot", {
                items: inventory,
            });
            console.log(`[WorldRoom] Inventory synchronized after pickup: ` +
                `player=${player.name} ` +
            ...
            // ─────────────────────────────────────────────────────────
            this.state.drops.delete(dropId);
            console.log(`[WorldRoom] GroundDrop picked up: ` +
                `${drop.itemName} (${drop.itemId}) ` +
                `quantity=${drop.quantity} ` +
                `player=${player.name} ` +
                `drop=${dropId}`);
            console.log(`[WorldRoom] GroundDrop deleted from state: ` +
                `drop=${dropId} ` +
                `exists=${this.state.drops.has(dropId)}`);
        }
        finally {
            this.processingDropPickups.delete(dropId);
        }
    }
    // ─────────────────────────────────────────────────────────────
    // ROOM CREATE
    ...
    async onCreate(_options) {
        ...
        this.onMessage("learnSkill", async (client, data) => {
            try {
                ...
                const result = await this.characterSkillService.learnSkill(player.characterId, skillId);
                ...
                console.log(`[WorldRoom] Skill learned: ` +
                ...
                const reason = error instanceof Error
                    ? error.message
                    : "UNKNOWN_ERROR";
                console.warn(`[WorldRoom] Failed to learn skill: ` +
                    `session=${client.sessionId} ` +
                    `reason=${reason}`);
                client.send("skillLearnResult", {
                    success: false,
        ...
        this.onMessage("useItem", async (client, data) => {
            try {
                ...
                const result = await itemUseService.useItem(player, inventoryId, () => this.recalculatePlayerStats(player));
                if (!result.success) {
                    client.send("itemUseResult", result);
                    return;
                }
                const inventory = await inventoryService.getInventory(player.characterId);
                client.send("inventorySnapshot", {
                ...
                console.error("[WorldRoom] useItem error:", error);
                client.send("itemUseResult", {
                    success: false,
        ...
        // ───────────────────────────────────────────────────────────
        this.onMessage("equipItem", async (client, data) => {
            try {
                const player = this.state.players.get(client.sessionId);
                if (!player) {
                    client.send("equipmentResult", {
                        success: false,
                        reason: "PLAYER_NOT_FOUND",
                    });
                    return;
                ...
                const equipment = await equipmentService.equipItem(player.characterId, inventoryId);
                ...
                console.log(`[WorldRoom] Item equipped: ` +
                    `player=${player.name} ` +
                ...
                const reason = error instanceof Error
                    ? error.message
                    : "UNKNOWN_ERROR";
                console.warn(`[WorldRoom] Failed to equip item: ` +
                    `session=${client.sessionId} ` +
                    `reason=${reason}`);
                client.send("equipmentResult", {
                    success: false,
        ...
        // ───────────────────────────────────────────────────────────
        this.onMessage("unequipItem", async (client, data) => {
            try {
                const player = this.state.players.get(client.sessionId);
                if (!player) {
                    client.send("equipmentResult", {
                ...
                const equipment = await equipmentService.unequipItem(player.characterId, slot);
                ...
                console.log(`[WorldRoom] Item unequipped: ` +
                    `player=${player.name} ` +
                    `slot=${slot}`);
            }
            catch (error) {
                const reason = error instanceof Error
                    ? error.message
                    : "UNKNOWN_ERROR";
                console.warn(`[WorldRoom] Failed to unequip item: ` +
                    `session=${client.sessionId} ` +
                    `reason=${reason}`);
                client.send("equipmentResult", {
                    success: false,
        ...
        this.onMessage("distributeStat", async (client, data) => {
            ...
            if (typeof data?.stat !== "string" ||
                !validStats.includes(data.stat)) {
            ...
            try {
                ...
                // ─────────────────────────────────────────────────
                const result = this.statSystem.increaseStat(currentStats, player.availablePoints, stat, amount);
                // ─────────────────────────────────────────────────
                ...
                console.log(`[WorldRoom] ${player.name} distributed ` +
     ...
     * baseadas no rAthena.
     */
    async recalculatePlayerStats(player) {
        if (!player.characterId) {
            return;
        }
        const statusModifiers = statusService.getStatModifiers(player.characterId);
        const equipmentModifiers = await equipmentService.getStatModifiers(player.characterId);
        const modifiers = {
            ...statusModifiers,
        ...
        //
        // será utilizado aqui.
        const combatModifiers = statusService.getCombatModifiers(player.characterId);
        const equipment = await equipmentService.getEquipment(player.characterId);
        const weapon = equipment.find((e) => e.item.type === "Weapon" &&
            e.slot === "Right_Hand") ??
            equipment.find((e) => e.item.type === "Weapon" &&
                e.slot === "Both_Hand") ??
            equipment.find((e) => e.item.type === "Weapon" &&
                e.slot === "Left_Hand");
        const weaponType = weapon?.item.subType ?? "Fist";
        const baseASPD = await this.statSystem.getBaseASPD(player.jobKey, weaponType);
        player.aspd =
            this.statSystem.calculateASPD(baseASPD, player.agi, player.dex, combatModifiers.aspdFixedBonus);
        // ─────────────────────────────────────────────────────────
        // CURRENT HP
        // ─────────────────────────────────────────────────────────
        //
        ...
        console.log(`[WorldRoom] Stats recalculated: ${player.characterId}`, {
            str: player.str,
            agi: player.agi,
            vit: player.vit,
            int: player.int,
            ...
            crit: player.crit,
            baseASPD,
            aspdFixedBonus: combatModifiers.aspdFixedBonus,
            aspd: player.aspd,
        });
    }
    // ─────────────────────────────────────────────────────────────
    ...
    spawnInitialMobs(mapDef) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const rMob = gameDataService.getMob(dbId);
            const mob = new Mob();
            mob.id =
    ...
    updateLoop(_deltaTime) {
        const now = Date.now();
        const expiredStatuses = statusService.update(now);
        if (expiredStatuses.length > 0) {
            for (const player of this.state.players.values()) {
                if (!player.characterId) {
                    continue;
                }
                const hasExpiredStatus = expiredStatuses.some((entry) => entry.startsWith(`${player.characterId}:`));
                if (!hasExpiredStatus) {
                    continue;
                }
        ...
        this.state.mobs.forEach((mob) => {
            ...
            if (Math.random() <
                ...
                !mob.targetId) {
                mob.targetX =
                    Math.max(60, Math.min(1200, mob.x +
                        (Math.random() *
                            200 -
                            100)));
                mob.targetY =
                    Math.max(60, Math.min(1200, mob.y +
                        (Math.random() *
                            200 -
                            100)));
            }
    ...
    async processPlayerAttack(player, mob, _sessionId) {
        ...
        // COMBAT SNAPSHOTS
        // ───────────────────────────────────────────────────────────
        const playerSnapshot = await this.combatStateBuilder.buildPlayerSnapshot({
            id: player.characterId,
            name: player.name,
        ...
        const mobData = gameDataService.getMob(mob.mobDbId);
        if (!mobData) {
            console.warn(`[WorldRoom] Player attack rejected: ` +
                `mob data not found ` +
                `mobDbId=${mob.mobDbId} ` +
                `mob=${mob.name}`);
            return;
        }
        const mobSnapshot = this.combatStateBuilder.buildMobSnapshot(mobData);
        // ───────────────────────────────────────────────────────────
        // CRITICAL
        // ───────────────────────────────────────────────────────────
        const isCrit = Math.random() *
            100 <
            playerSnapshot.combatStats.crit;
        // ───────────────────────────────────────────────────────────
        // HIT / FLEE
        // ───────────────────────────────────────────────────────────
        const hitRate = playerSnapshot.combatStats.hit;
        const fleeRate = mobSnapshot.stats.flee;
        ...
        const components = resolveBasicAttackComponents(playerSnapshot, {
            targetSize: mobSnapshot.size,
            randomValue: Math.random(),
            overRefineRandomValue: Math.random(),
        });
        // ───────────────────────────────────────────────────────────
        // DAMAGE
        // ───────────────────────────────────────────────────────────
        ...
        // usesAmmo = false
        //
        const damageResult = combatSystem.performWeaponAttack({
            attacker: playerSnapshot,
            target: mobSnapshot,
            components,
            attackElement: resolveEffectiveAttackElement(playerSnapshot.weapon?.element ?? "Neutral", {}),
            targetElement: mobSnapshot.element,
            targetElementLevel: mobSnapshot.elementLevel,
            skillRatio: 100,
            skillConstant: 0,
            skillId: 0,
            isCritical: isCrit,
        ...
        if (mob.hp <= 0) {
            ...
            // DROPS
            // ─────────────────────────────────────────────────────────
            const rolledDrops = dropService.rollDrops(mob.mobDbId);
            for (const drop of rolledDrops) {
                const item = gameDataService.getItem(drop.itemId);
                if (!item) {
                    console.warn(`[WorldRoom] Drop item not found: ` +
                        `mob=${mob.mobDbId} ` +
                        `item=${drop.itemId} ` +
                        `source=${drop.sourceKey ?? "unknown"}`);
                    continue;
                }
                console.log(`[WorldRoom] Drop rolled: ` +
                    `mob=${mob.name} (${mob.mobDbId}) ` +
                    `item=${item.name} (${item.id}) ` +
                    `rate=${drop.rate} ` +
                    `source=${drop.sourceKey ?? "unknown"}`);
                this.spawnGroundDrop(item.id, item.name, 1, mob.x, mob.y, player.id);
                this.broadcast("combatLog", {
            ...
            const jobExpGained = mob.jobExp;
            if (jobExpGained > 0) {
                const jobResult = await this.jobProgressionService.gainJobExp(player.characterId, BigInt(jobExpGained));
                player.jobLevel =
                    jobResult.jobLevel;
                player.jobExp =
                ...
                this.broadcast("combatLog", {
                    text: `Ganhou ${jobExpGained} Job EXP.`,
                    color: "#66ccff",
                });
                if (jobResult.jobLevelUps > 0) {
                    this.broadcast("combatLog", {
                        text: `${player.name} SUBIU ` +
                            `PARA O JOB LEVEL ` +
                            `${player.jobLevel} ! ` +
    ...
    async onJoin(client, options) {
        ...
        console.log(client.sessionId, "joining world with characterId:", options?.characterId);
        ...
        try {
            ...
            player.hp =
                Math.min(char.hp, player.maxHp);
            player.mp =
                Math.min(char.mp, player.maxMp);
            // ─────────────────────────────────────────────────────────
            // POSITION
            // ─────────────────────────────────────────────────────────
            player.x =
            ...
            console.log(`[WorldRoom] Inventory sent: ` +
                `player=${player.name} ` +
                `entries=${inventory.length}`);
            const equipment = await equipmentService.getEquipment(player.characterId);
            client.send("equipmentSnapshot", {
            ...
            console.log(`[WorldRoom] Equipment sent: ` +
                `player=${player.name} ` +
    ...
    async onLeave(client, _consented) {
        ...
        if (player &&
            player.characterId) {
            statusService.clear(player.characterId);
            try {
                await prisma.character.update({
                    where: {
                        id: player.characterId,
