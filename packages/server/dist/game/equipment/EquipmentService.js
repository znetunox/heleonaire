...
import { itemScriptInterpreter, } from "../items/ItemScriptInterpreter";
...
export class EquipmentService {
    ...
    parseLocations(locations) {
        ...
        for (const location of activeLocations) {
            if (!VALID_LOCATIONS.has(location)) {
                throw new Error(`Unsupported equipment location: ${location}`);
            }
        }
    ...
    async equipItem(characterId, inventoryId) {
        ...
        if (inventory.quantity < 1) {
            throw new Error("Inventory item has no quantity");
        }
        const item = inventory.item;
        const character = inventory.character;
        const locations = this.parseLocations(item.locations);
        if (locations.length === 0) {
            throw new Error("Item cannot be equipped");
        }
        this.validateLevel(character.level, item.equipLevelMin, item.equipLevelMax);
        await this.validateJob(character.jobKey, item.jobs);
        const occupiedSlots = [
            ...new Set(locations.flatMap((location) => location.occupies)),
        ];
     ...
     */
    async getWeaponContext(characterId) {
        if (!characterId) {
            throw new Error("Character ID is required");
        }
        ...
        const weaponEquipment = equipment.find((entry) => entry.slot === "Both_Hand" &&
            entry.item.type === "Weapon") ??
            equipment.find((entry) => entry.slot === "Right_Hand" &&
                entry.item.type === "Weapon");
        if (!weaponEquipment) {
            return null;
        }
        ...
        if (weaponEquipment.item.script) {
            const effects = itemScriptInterpreter.interpret(weaponEquipment.item.script);
        ...
        if (weaponEquipment.item.attack === null) {
            throw new Error(`Weapon ${weaponEquipment.item.aegisName} has no attack value`);
        }
        if (weaponEquipment.item.weaponLevel === null) {
            throw new Error(`Weapon ${weaponEquipment.item.aegisName} has no weapon level`);
        }
        let refineLevel = weaponEquipment.refineLevel;
        if (weaponEquipment.inventoryId) {
            ...
            if (!inventory) {
                throw new Error(`Weapon inventory instance ${weaponEquipment.inventoryId} not found`);
            }
            if (inventory.itemId !== weaponEquipment.itemId) {
                throw new Error(`Weapon inventory instance ${inventory.id} does not match equipped item ${weaponEquipment.itemId}`);
            }
            refineLevel =
                inventory.refineLevel;
        }
        if (refineLevel < 0) {
            throw new Error(`Invalid weapon refine level: ${refineLevel}`);
        }
        let refineBonus = 0;
        let overRefineBonus = 0;
        if (refineLevel > 0) {
            const refineRule = await prisma.refineRule.findUnique({
                where: {
                    group_itemLevel_refineLevel: {
                        group: "Weapon",
                        itemLevel: weaponEquipment.item.weaponLevel,
                        refineLevel,
                    },
                },
                select: {
            ...
            if (!refineRule) {
                throw new Error(`Refine rule not found for Weapon level ${weaponEquipment.item.weaponLevel} refine +${refineLevel}`);
            }
            refineBonus =
        ...
        return {
            ...
            inventoryId: weaponEquipment.inventoryId,
            attack: weaponEquipment.item.attack,
            weaponAtkBonus,
            weaponAtk2Bonus,
            weaponLevel: weaponEquipment.item.weaponLevel,
            weaponType: weaponEquipment.item.subType ?? "",
            range: weaponEquipment.item.range ?? 0,
            refineLevel,
            refineBonus,
            overRefineBonus,
            element: "Neutral",
     ...
     */
    async getStatModifiers(characterId) {
        if (!characterId) {
            throw new Error("Character ID is required");
        }
        const equipment = await prisma.characterEquipment.findMany({
        ...
        for (const equipped of equipment) {
            const key = equipped.inventoryId ??
                `legacy:${equipped.itemId}`;
            if (!equippedInstances.has(key)) {
                equippedInstances.set(key, equipped.item);
            }
        }
        let weaponAtk = 0;
        let equipAtk = 0;
        ...
        for (const item of equippedInstances.values()) {
            ...
            const effects = itemScriptInterpreter.interpret(item.script);
            for (const effect of effects) {
                switch (effect.type) {
                    /**
                     * bBaseAtk
                    ...
                    case "weaponDamageRate": {
                        const weaponType = effect.weaponType;
                        if (!weaponType) {
                            break;
                        }
                        const current = weaponDamageRateByType.get(weaponType) ?? 0;
                        weaponDamageRateByType.set(weaponType, current + effect.value);
                        break;
                    }
                    case "def":
                        def += effect.value;
                    ...
                    case "weaponAtkByType": {
                        ...
                        const current = weaponAtkByType.get(weaponType) ?? 0;
                        weaponAtkByType.set(weaponType, current + effect.value);
                        break;
                    }
                    case "weaponAtk":
                    case "weaponAtk2":
        ...
        return {
            ...
            atkRate,
            weaponAtkRate,
            weaponDamageRateByType: Object.fromEntries(weaponDamageRateByType),
    ...
    validateLevel(characterLevel, minLevel, maxLevel) {
        if (minLevel !== null &&
            characterLevel < minLevel) {
            throw new Error(`Character level ${characterLevel} is below required level ${minLevel}`);
        }
        if (maxLevel !== null &&
            characterLevel > maxLevel) {
            throw new Error(`Character level ${characterLevel} exceeds maximum level ${maxLevel}`);
        }
    }
    /**
     ...
     */
    async validateJob(characterJobKey, jobsJson) {
        if (!jobsJson) {
            return;
        }
        let jobs;
        try {
            ...
            throw new Error("Invalid item jobs data");
        }
        const entries = Object.entries(jobs);
        if (entries.length === 0) {
            return;
        }
        const normalizedJobKey = characterJobKey.trim().toLowerCase();
        const explicitFalseJobs = entries
            .filter(([, value]) => value === false)
            .map(([job]) => job);
        const isExplicitlyDenied = await this.characterMatchesJobHierarchy(characterJobKey.trim(), explicitFalseJobs);
        if (isExplicitlyDenied) {
            throw new Error(`Job ${characterJobKey} cannot equip this item`);
        }
        const allEntry = entries.find(([job]) => job.trim().toLowerCase() === "all");
        if (allEntry?.[1] === true) {
            return;
        }
        const allowedJobs = entries
            .filter(([, value]) => value === true)
            .map(([job]) => job)
            .filter((job) => job.trim().toLowerCase() !== "all");
        if (allowedJobs.length === 0) {
            throw new Error(`Job ${characterJobKey} cannot equip this item`);
        }
        const allowed = await this.characterMatchesJobHierarchy(characterJobKey.trim(), allowedJobs);
        if (!allowed) {
            throw new Error(`Job ${characterJobKey} cannot equip this item`);
        }
    }
    async characterMatchesJobHierarchy(characterJobKey, targetJobs) {
        const normalizedTargets = new Set(targetJobs.map((job) => job.trim().toLowerCase()));
        let currentJobKey = characterJobKey.trim();
        const visited = new Set();
        while (currentJobKey) {
            const normalizedCurrent = currentJobKey.toLowerCase();
            if (visited.has(normalizedCurrent)) {
                throw new Error(`Circular GameClass hierarchy detected at ${currentJobKey}`);
            }
            ...
            const parent = await prisma.gameClass.findUnique({
                where: {
