...
export class ItemUseService {
    async useItem(player, inventoryId, recalculateStats) {
        ...
        const script = inventoryEntry.item.script;
        if (!script) {
            return {
                success: false,
        ...
        for (const effect of effects) {
            switch (effect.type) {
                ...
                case "STATUS_START":
                    ...
                    break;
                case "STATUS_END":
                    if (statusService.hasStatus(player.characterId, effect.statusId)) {
        ...
        const consumed = await inventoryService.consumeItem(player.characterId, inventoryId, 1);
        if (!consumed) {
            return {
                success: false,
        ...
        for (const effect of effects) {
            ...
            statusService.addStatus(player.characterId, effect.statusId, effect.durationMs, effect.value, effect.value2, effect.value3, effect.value4, {
                type: "ITEM",
        ...
        for (const effect of effects) {
            ...
            statusService.removeStatus(player.characterId, effect.statusId);
        }
        /*
        ...
        for (const effect of effects) {
            switch (effect.type) {
                case "HEAL":
                    newHp =
                        Math.min(player.maxHp, Math.max(0, newHp + effect.hp));
                    newMp =
                        Math.min(player.maxMp, Math.max(0, newMp + effect.mp));
                    break;
                case "PERCENT_HEAL":
                    newHp =
                        Math.min(player.maxHp, Math.max(0, newHp +
                            Math.floor(player.maxHp *
                                effect.hpPercent /
                                100)));
                    newMp =
                        Math.min(player.maxMp, Math.max(0, newMp +
                            Math.floor(player.maxMp *
                                effect.mpPercent /
                                100)));
                    break;
                case "STATUS_START":
        ...
        const hpDelta = newHp - hpBefore;
        const mpDelta = newMp - mpBefore;
        /*
        ...
        player.hp = newHp;
        player.mp = newMp;
        console.log(`[ItemUseService] Item used: ` +
    ...
    assertNeverEffect(effect) {
        throw new Error(`Unsupported item effect: ${JSON.stringify(effect)} `);
    }
}
export const itemUseService = new ItemUseService();
