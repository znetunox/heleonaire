export function calculateASPD({ baseASPD, agi, dex, aspdFixedBonus = 0, }) {
    if (!Number.isFinite(baseASPD) || baseASPD <= 0) {
        throw new Error(`Invalid baseASPD: ${baseASPD}`);
    }
    if (!Number.isFinite(agi) || agi < 0) {
        throw new Error(`Invalid agi: ${agi}`);
    }
    if (!Number.isFinite(dex) || dex < 0) {
        throw new Error(`Invalid dex: ${dex}`);
    }
    if (!Number.isFinite(aspdFixedBonus) ||
        aspdFixedBonus < 0) {
        throw new Error(`Invalid aspdFixedBonus: ${aspdFixedBonus}`);
    }
    /*
     ...
     * sqrt(temp) * 0.25 + 196
     */
    const tempASPD = Math.sqrt(tempBase) * 0.25 +
        196;
    /*
     * rAthena:
     *
     ...
     * min(baseASPD, 200)
     */
    const aspd = Math.floor(withFixedBonus) -
     ...
     */
    const amotion = 2000 - aspd * 10;
    const adelay = amotion / 2;
    /*
...
export class StatSystem {
    ...
    calculateASPD(baseASPD, agi, dex, aspdFixedBonus = 0) {
        if (!Number.isFinite(baseASPD) || baseASPD <= 0) {
            throw new Error(`Invalid baseASPD: ${baseASPD}`);
        }
        if (!Number.isFinite(agi) || agi < 0) {
         ...
         *     min(baseASPD, 200)
         */
        const aspd = Math.floor(modifiedASPD) -
            Math.min(baseASPD, 200);
        /*
         * Renewal:
         *
         ...
         */
        const amotion = 2000 - aspd * 10;
        const adelay = Math.floor(amotion / 2);
    ...
    */
    async getBaseASPD(jobKey, weaponType = "Fist") {
        if (!jobKey) {
            throw new Error("jobKey is required.");
        }
        const gameClass = await this.prisma.gameClass.findUnique({
            where: {
                aegisName: jobKey,
            },
            select: {
        ...
        if (!gameClass) {
            throw new Error(`GameClass not found for jobKey = ${jobKey}.`);
        }
        const classAspd = await this.prisma.classAspd.findUnique({
            where: {
                classId_weaponType: {
        ...
        if (!classAspd) {
            throw new Error(`ASPD not found for ` +
                `job = ${jobKey} ` +
     ...
     */
    calculateDerivedStats(stats, level, modifiers = {}) {
        ...
        const maxMp = Math.max(1, Math.floor(maxMpBase *
            (1 + (modifiers.maxMpPercent ?? 0) / 100)));
        const batk = effectiveStats.str +
        ...
        return {
            maxHp,
            maxMp,
            atk: effectiveStats.str * 2 +
                Math.floor(effectiveStats.dex / 5) +
                (modifiers.atk ?? 0),
            batk,
            statusAtk,
            patk,
            matk: effectiveStats.int * 2 +
                Math.floor(effectiveStats.dex / 5) +
                (modifiers.matk ?? 0),
            def1: Math.floor((modifiers.defense ?? 0) *
                (100 + (modifiers.defenseRate ?? 0)) /
                100),
            def2: Math.floor((Math.floor((level + effectiveStats.vit) / 2)
                + Math.floor(effectiveStats.agi / 5)
                + (modifiers.defense2 ?? 0)) *
                (100 + (modifiers.defense2Rate ?? 0)) /
                100),
            mdef1: modifiers.magicDefense ?? 0,
            mdef2: effectiveStats.int +
                Math.floor(level / 4) +
            ...
            flee: 100 +
                ...
                (modifiers.flee ?? 0),
            crit: Math.floor(effectiveStats.luk / 3) +
                (modifiers.crit ?? 0),
        };
    }
}
