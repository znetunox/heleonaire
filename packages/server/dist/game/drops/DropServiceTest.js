...
async function testDropService() {
    ...
    if (entries.length !== 8) {
        throw new Error(`Poring deveria possuir 8 entradas, encontrou ${entries.length}.`);
    }
    console.log("\nExecutando 10 rolls:\n");
    for (let i = 1; i <= 10; i++) {
        ...
        for (const drop of drops) {
            ...
            console.log(`  ${item?.name ?? `Item ${drop.itemId}`} ` +
    ...
    for (let i = 0; i < iterations; i++) {
        const drops = dropService.rollDrops(mobId);
        for (const drop of drops) {
            counters.set(drop.itemId, (counters.get(drop.itemId) ?? 0) + 1);
        }
    }
    for (const entry of entries) {
        ...
        console.log(`${item?.name ?? entry.itemId}: ` +
