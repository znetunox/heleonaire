...
if (orderedAttacker.weaponAtk !== 74 ||
    orderedAttacker.equipAtk !== 37) {
    throw new Error(`Attacker order/truncation failed: ${JSON.stringify(orderedAttacker)}`);
}
...
if (race2Attacker.weaponAtk !== 73 ||
    race2Attacker.equipAtk !== 73) {
    throw new Error(`Attacker Race2 aggregation failed: ${JSON.stringify(race2Attacker)}`);
}
const preserved = {
    statusAtk: 123,
    weaponAtk: 100,
...
if (ignoredAttacker.weaponAtk !== 100 ||
...
if (ignoredElement.weaponAtk !== 81 ||
    ignoredElement.equipAtk !== 81) {
    throw new Error(`ignoreElementCardfix was not isolated: ${JSON.stringify(ignoredElement)}`);
}
...
if (orderedDefender.damage !== 490) {
    throw new Error(`Defender order/truncation failed: ${JSON.stringify(orderedDefender)}`);
}
...
if (ignoredDefenderElement.damage !== 81) {
    throw new Error(`Defender ignoreElementCardfix was not isolated: ${JSON.stringify(ignoredDefenderElement)}`);
}
...
if (combinedFlags.weaponAtk !== 100 ||
