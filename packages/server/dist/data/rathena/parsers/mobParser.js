...
export function parseMobs() {
    console.log("[rAthena] Reading mob_db.yml...");
    const resolved = resolveYaml("mob_db.yml");
    const body = Array.isArray(resolved.data)
        ? resolved.data
        : resolved.data?.Body ?? [];
    console.log(`[rAthena] mob_db.yml source: ${resolved.source}`);
    console.log(`[rAthena] mob_db.yml entries: ${body.length}`);
    const mobs = new Map();
    let duplicates = 0;
