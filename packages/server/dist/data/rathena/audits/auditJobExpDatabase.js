...
async function main() {
    ...
    if (sourceGroups.length !== dbGroups.length) {
        errors.push(`Group count mismatch: source=${sourceGroups.length}, db=${dbGroups.length}`);
    }
    // ============================================================
    // GROUPS
    // ============================================================
    const dbGroupsByKey = new Map(dbGroups.map((group) => [group.sourceKey, group]));
    ...
    for (const sourceGroup of sourceGroups) {
        ...
        if (!dbGroup) {
            errors.push(`Missing group: sourceKey=${sourceKey}`);
            continue;
        }
        ...
        if (dbGroup.maxJobLevel !== sourceGroup.maxJobLevel) {
            errors.push(`MaxJobLevel mismatch: ${sourceKey} ` +
        ...
        if (dbGroup.entries.length !== sourceGroup.entries.length) {
            errors.push(`Entry count mismatch: ${sourceKey} ` +
        ...
        // Entries
        // ----------------------------------------------------------
        const dbEntries = new Map(dbGroup.entries.map((entry) => [entry.level, entry]));
        for (const sourceEntry of sourceGroup.entries) {
            const dbEntry = dbEntries.get(sourceEntry.level);
            if (!dbEntry) {
                errors.push(`Missing entry: ${sourceKey} level=${sourceEntry.level}`);
                continue;
            }
            if (!bigintEqual(dbEntry.exp, sourceEntry.exp)) {
                errors.push(`EXP mismatch: ${sourceKey} level=${sourceEntry.level} ` +
        ...
        const expectedHash = createHash("sha256")
            .update(JSON.stringify(expectedHashInput, (_key, value) => typeof value === "bigint"
            ? value.toString()
            : value))
            .digest("hex");
        if (dbGroup.sourceHash !== expectedHash) {
            errors.push(`Hash mismatch: ${sourceKey}`);
        }
    }
    // ============================================================
    // EXTRA GROUPS
    // ============================================================
    const sourceKeys = new Set(sourceGroups.map((group) => createSourceKey(group.jobs)));
    for (const dbGroup of dbGroups) {
        if (!sourceKeys.has(dbGroup.sourceKey)) {
            errors.push(`Extra DB group: sourceKey=${dbGroup.sourceKey}`);
        }
    }
    // ============================================================
    // ENTRY TOTAL
    ...
    if (expectedEntries !== actualEntries) {
        errors.push(`Total entry mismatch: source=${expectedEntries}, db=${actualEntries}`);
    }
    // ============================================================
    ...
    for (const job of playableJobs) {
        ...
        const sourceGroup = sourceGroups.find((group) => group.jobs.some((groupJob) => normalizeJobName(groupJob) === jobName));
        if (!sourceGroup) {
            errors.push(`Source Job EXP group missing for class: ${jobName}`);
            continue;
        }
        const sourceKey = createSourceKey(sourceGroup.jobs);
        ...
        if (!dbClass) {
            errors.push(`GameClass missing: ${jobName}`);
            continue;
        }
        if (!dbClass.jobExpGroupId) {
            errors.push(`GameClass not linked: ${jobName}`);
            continue;
        }
        if (!dbClass.jobExpGroup) {
            errors.push(`GameClass linked to missing group: ${jobName}`);
            continue;
        }
        if (dbClass.jobExpGroup.sourceKey !== sourceKey) {
            errors.push(`Wrong Job EXP group: ${jobName} ` +
                `expected=${sourceKey} ` +
    ...
    if (linkedClasses !== playableJobs.length) {
        errors.push(`Linked class count mismatch: expected=${playableJobs.length}, actual=${linkedClasses}`);
    }
    // ============================================================
    // SUMMARY
    // ============================================================
