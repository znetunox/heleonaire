...
function prerequisiteSignature(skill) {
...
function prerequisiteDisplay(skill) {
    if (skill.requires.length === 0) {
        return "[]";
    }
    return JSON.stringify([...skill.requires].sort((a, b) => {
        const nameCompare = a.name.localeCompare(b.name);
        if (nameCompare !== 0) {
            return nameCompare;
        }
...
async function main() {
    ...
    for (const jobName of HELEONAIRE_JOBS) {
        if (!jobMap.has(jobName)) {
            throw new Error(`Missing skill tree for Heleonaire job: ${jobName}`);
        }
    ...
    for (const jobName of HELEONAIRE_JOBS) {
        ...
        console.log(`  ${jobName.padEnd(20)} ${skills.length}`);
    }
    console.log("");
    ...
    for (const [skillName, list] of occurrences) {
        ...
        for (const occurrence of list) {
            ...
            const signature = prerequisiteSignature(occurrence.skill);
            ...
            if (existing) {
                ...
                variants.set(signature, {
                    jobs: [occurrence.job],
                    display: prerequisiteDisplay(occurrence.skill),
                });
            }
        }
        if (variants.size > 1) {
            prerequisiteConflicts.push({
                skill: skillName,
                variants: [...variants.entries()].map(([signature, data]) => ({
                    signature,
                    jobs: data.jobs,
    ...
    console.log(`Total prerequisite entries: ${totalPrerequisites}`);
    console.log(`Skills with conflicting prerequisites: ${prerequisiteConflicts.length}`);
    if (prerequisiteConflicts.length > 0) {
        ...
        for (const conflict of prerequisiteConflicts) {
            ...
            for (const variant of conflict.variants) {
                console.log(`  Jobs: ${variant.jobs.join(", ")}`);
                console.log(`  Requires: ${variant.display}`);
            }
            console.log("");
        }
    }
    else {
        console.log("OK: prerequisites are globally consistent.");
    }
    // ---------------------------------------------------------
    ...
    for (const [skillName, list] of occurrences) {
        for (const occurrence of list) {
            for (const requirement of occurrence.skill.requires) {
                const key = `${skillName} -> ${requirement.name}`;
                ...
                const jobs = pair.levels.get(requirement.level) ?? [];
                jobs.push(occurrence.job);
                pair.levels.set(requirement.level, jobs);
            }
        }
    }
    const pairConflicts = [...pairLevels.values()]
        .filter(pair => pair.levels.size > 1);
    console.log(`Prerequisite pairs: ${pairLevels.size}`);
    console.log(`Pairs with conflicting levels: ${pairConflicts.length}`);
    if (pairConflicts.length > 0) {
        console.log("");
        console.log("!!! LEVEL CONFLICTS !!!");
        console.log("");
        for (const pair of pairConflicts) {
            console.log(`${pair.skill} -> ${pair.requiredSkill}`);
            for (const [level, jobs] of pair.levels) {
                console.log(`  Level ${level}: ${jobs.join(", ")}`);
            }
            console.log("");
        }
    }
    else {
        console.log("OK: prerequisite pair levels are consistent.");
    }
    // ---------------------------------------------------------
    // Audit BaseLevel
    // ---------------------------------------------------------
    ...
    for (const [skillName, list] of occurrences) {
        for (const occurrence of list) {
            const baseLevel = occurrence.skill.baseLevel;
            if (baseLevel === undefined) {
                continue;
            }
            baseLevelCount++;
            const key = String(baseLevel);
            const variants = baseLevelVariants.get(skillName) ??
                new Map();
            const jobs = variants.get(key) ?? [];
            jobs.push(occurrence.job);
            variants.set(key, jobs);
            baseLevelVariants.set(skillName, variants);
        }
    }
    const baseLevelConflicts = [...baseLevelVariants.entries()]
        .filter(([, variants]) => variants.size > 1);
    console.log(`Skill occurrences with BaseLevel: ${baseLevelCount}`);
    console.log(`Skills with conflicting BaseLevel: ${baseLevelConflicts.length}`);
    if (baseLevelConflicts.length > 0) {
        ...
        for (const [skillName, variants] of baseLevelConflicts) {
            ...
            for (const [level, jobs] of variants) {
                console.log(`  BaseLevel ${level}: ${jobs.join(", ")}`);
            }
            console.log("");
        }
    }
    else {
        console.log("OK: BaseLevel is consistent wherever present.");
    }
    // ---------------------------------------------------------
    // Audit JobLevel
    // ---------------------------------------------------------
    ...
    for (const [skillName, list] of occurrences) {
        for (const occurrence of list) {
            const jobLevel = occurrence.skill.jobLevel;
            if (jobLevel === undefined) {
                continue;
            }
            const key = String(jobLevel);
            const variants = jobLevelVariants.get(skillName) ??
                new Map();
            const jobs = variants.get(key) ?? [];
            jobs.push(occurrence.job);
            variants.set(key, jobs);
            jobLevelVariants.set(skillName, variants);
        }
    }
    const jobLevelConflicts = [...jobLevelVariants.entries()]
        .filter(([, variants]) => variants.size > 1);
    console.log(`Skills with conflicting JobLevel: ${jobLevelConflicts.length}`);
    if (jobLevelConflicts.length > 0) {
        console.log("");
        for (const [skillName, variants] of jobLevelConflicts) {
            console.log(`Skill: ${skillName}`);
            for (const [level, jobs] of variants) {
                console.log(`  JobLevel ${level}: ${jobs.join(", ")}`);
            }
            console.log("");
        }
    }
    else {
        console.log("OK: JobLevel is consistent wherever present.");
    }
    // ---------------------------------------------------------
    // Final result
    // ---------------------------------------------------------
    ...
    let failed = false;
    if (prerequisiteConflicts.length > 0) {
        console.log("FAIL: prerequisites are class-specific.");
        failed = true;
    }
    if (pairConflicts.length > 0) {
        console.log("FAIL: same prerequisite pair has different levels.");
        failed = true;
    }
    if (baseLevelConflicts.length > 0) {
        console.log("FAIL: BaseLevel is class-specific.");
        failed = true;
    }
    if (failed) {
        console.log("");
        console.log("Schema changes are required before import.");
        console.log("==============================================");
        process.exitCode = 1;
        return;
    }
    console.log("PASS: current model can represent the effective skill trees.");
    console.log("");
    console.log(`ClassSkill rows: ${totalClassSkills}`);
    console.log(`Prerequisite rows: ${totalPrerequisites}`);
    console.log("==============================================");
}
main().catch(error => {
    console.error("");
